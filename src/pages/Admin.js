...
import { Shield, Users, Search, Crown, Clock, Star, RefreshCw, AlertTriangle, XCircle } from 'lucide-react'
...
export default function Admin() {
  ...
  const grantPro = async (userId, trialDays = 0) => {
    ...
    try {
      ...
      toast.success(trialDays > 0 ? `${trialDays}-day trial granted` : 'Pro access granted')
      fetchData()
    } catch (err) {
            ...
            {filteredUsers.map(u => {
              ...
              return (
                <tr key={u.id}>
                  ...
                  <td className="td-muted">{trialEnd ? new Date(trialEnd).toLocaleDateString() : '—'}</td>
                  <td>
                    <div className="action-btns">
                      {!isPro ? (
                        <>
                          <button className="btn btn-primary btn-sm" onClick={() => grantPro(u.id)} disabled={isUpdating}>
                            <Crown size={12} /> Grant Pro
                          </button>
                          <button className="btn btn-secondary btn-sm" onClick={() => grantPro(u.id, 14)} disabled={isUpdating}>
                            14-Day Trial
                          </button>
                          <button className="btn btn-secondary btn-sm" onClick={() => grantPro(u.id, 30)} disabled={isUpdating}>
                            30-Day Trial
                          </button>
                        </>
                      ) : (
